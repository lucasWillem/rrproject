...
} from '../../../hooks/useFetchAllCountries';
...
const CountryList: FC<CountryListProps> = ({ limit }) => {
  const {
    ...
    isFetching: isFetchingListOfCountries,
    error: errorFetchingCountryList,
  } = useFetchAllCountries(limit);
  ...
  useEffect(() => {
    dispatch(
      showLoader({ isVisible: isFetchingListOfCountries, variant: 'inherit' }),
    );
  ...
  const renderItem = (country: BasicCountry, i: number) => (
    <CountryCard
      data-testid="country-item"
      key={i}
      cardVariant={CardVariant.Basic}
      raised
      country={country}
  ...
  return (
    <Box>
      {listOfCountries?.map((country: BasicCountry, i) =>
