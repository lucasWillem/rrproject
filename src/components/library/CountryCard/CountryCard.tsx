...
import { FaUserFriends, FaComment, FaBuilding, FaDollarSign, FaGlobe } from 'react-icons/fa';
...
const CountryCard: FC<CountryCardProps> = (props) => {
  ...
  const renderBasicItems = useCallback((item: any, i: number, arr: any[], typographyVariant: TypographyVariant) =>
    <Typography variant={typographyVariant} key={`${item}-${i}`}>
      {item + (i < arr.length - 1 ? ',' : '')}
    </Typography>, [country]);

  const renderCurrencies = useCallback((item: any, i: number, arr: any[], typographyVariant: TypographyVariant) => {
    ...
    <StyledCountryCard cardVariant={cardVariant} style={containerStyle} raised {...restProps}>
      <CardContent>
...

        {
          ...
          <>
            <CountryDatumItem icon={capitalIcon} items={[(country as SpecificCountry).capital]} renderItem={renderBasicItems} typographyVariant={TypographyVariants.caption} /> 
            <CountryDatumItem icon={regionIcon} items={[(country as SpecificCountry).region]} renderItem={renderBasicItems} typographyVariant={TypographyVariants.caption} />
            <CountryDatumItem icon={languagesIcon} items={Object.values((country as SpecificCountry)['languages'])} renderItem={renderBasicItems} typographyVariant={TypographyVariants.caption} />
            <CountryDatumItem icon={currenciesIcon} items={Object.entries((country as SpecificCountry).currencies)} renderItem={renderCurrencies}
              typographyVariant={TypographyVariants.caption} />
            <CountryDatumItem icon={populationIcon} items={[(country as SpecificCountry).population]} renderItem={renderBasicItems} typographyVariant={TypographyVariants.caption} />
          </>
        }
